            ...
            foreach (RaceTeamData teamState in teamStatesSnapshot)
            {
                ...
                if (NetState.TryGetTeamData(teamState.TeamId, out RaceTeamData teamData))
                {
                    ...
                    && NetState.PlayerStates[teamData.ShotgunConnectionId].IsTrackReady)
                    {
                        ...
                        _netState.SetTeamData(teamState.TeamId, new RaceTeamData(NetState.TeamData[teamState.TeamId], teamNob: team.NetworkObject, driverNob: player.NetworkObject, shotgunNob: shotgun.NetworkObject, isTeamSpawned: true));
                        _spawnedTeams++;
                    }
                    else
                    {
